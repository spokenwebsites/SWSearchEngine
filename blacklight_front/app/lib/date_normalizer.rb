...
class DateNormalizer
  ...
  #   elsif date_string.match?(/\d{4}/)
  #     Date.strptime(date_string, '%Y').strftime('%Y').to_i
  #   elsif date_string.match?(/\d+ [a-zA-Z]+ \d{4}/)
  #     Date.strptime(date_string, '%d %B %Y').strftime('%Y').to_i
  #   else
  #     date_string.to_i
  #   end
  # rescue ArgumentError
  ...
  def self.year_from_date(date_string)
    ...
    year = parts.find { |part| part.match?(/\d{4}/) }
